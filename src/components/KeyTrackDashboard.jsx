...
function Ring({ pct, label, value, color = "#8B5CF6" }) {
  const r = 30;
  const c = 2 * Math.PI * r;
...
function AddLessonModal({ isOpen, onClose, onAdd, lang }) {
  ...
  // Prevent background scroll on wheel event
  useEffect(() => {
    ...
    >
      <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col animate-scale-up">
        ...
        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-100 flex-shrink-0">
          <h3 className="text-lg sm:text-xl font-bold text-gray-800">{mt.addNewLesson}</h3>
          <button
            onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 sm:gap-3">
                {["#8B5CF6", "#3B82F6", "#EC4899", "#06B6D4", "#F59E0B", "#10B981", "#EF4444", "#6366F1"].map((color) => (
                  <button
                    key={color}
                    type="button"
                    onClick={() => setFormData({...formData, color: color})}
                    className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full transition-transform ${
                      formData.color === color ? 'scale-110 ring-2 ring-offset-2 ring-gray-400' : 'hover:scale-110'
                    }`}
...
// Column Chart Component with translations
function ColumnChart({ lang }) {
  ...
  const chartTranslations = {
    ...
    ps: {
      ...
      hoursSpent: "پدې اونۍ کې مصرف شوي ساعتونه",
      days7: "۷ ورځې",
  ...
  const ct = chartTranslations[lang] || chartTranslations.en;
  
  const weeklyData = [
    { day: ct.days.Mon, hours: 2.5, color: "#8B5CF6" },
    { day: ct.days.Tue, hours: 3.8, color: "#3B82F6" },
    { day: ct.days.Wed, hours: 1.9, color: "#EC4899" },
    { day: ct.days.Thu, hours: 4.2, color: "#06B6D4" },
    { day: ct.days.Fri, hours: 3.1, color: "#F59E0B" },
    { day: ct.days.Sat, hours: 5.0, color: "#10B981" },
    { day: ct.days.Sun, hours: 2.8, color: "#6366F1" },
  ];
  ...
  return (
    <div className="w-full">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6 gap-3">
        <div>
          <h3 className="text-base sm:text-lg font-bold text-gray-800">{ct.weeklyActivity}</h3>
          <p className="text-xs sm:text-sm text-gray-500">{ct.hoursSpent}</p>
        </div>
        <div className="flex gap-2">
          <span className="text-[10px] sm:text-xs bg-purple-100 text-purple-600 px-2 sm:px-3 py-1 rounded-full font-semibold">{ct.days7}</span>
          <span className="text-[10px] sm:text-xs bg-gray-100 text-gray-600 px-2 sm:px-3 py-1 rounded-full font-semibold">{ct.total}</span>
        </div>
      </div>
      
      <div className="flex items-end justify-between gap-1 sm:gap-3 md:gap-4" style={{ height: `${chartHeight + 40}px` }}>
        {weeklyData.map((data, index) => {
          ...
          return (
            ...
            >
              ...
              <div className={`mt-1 sm:mt-2 text-[10px] sm:text-xs font-semibold transition-colors ${
      ...
      <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-4 mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-gray-100">
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <span className="text-[10px] sm:text-xs text-gray-600">{ct.practiceTime}</span>
        </div>
        <div className="flex items-center gap-1.5 sm:gap-2">
          <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-blue-500" />
          <span className="text-[10px] sm:text-xs text-gray-600">{ct.lessons}</span>
        </div>
        <div className="flex items-center gap-1.5 sm:gap-2">
          <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-pink-500" />
          <span className="text-[10px] sm:text-xs text-gray-600">{ct.exercises}</span>
        </div>
      </div>
    </div>
  );
...
export default function KeyTrackDashboard({ user = "Fazl Ahmad",openToPahshto }) {
  ...
  const [lessons, setLessons] = useState(() => {
    ...
    if (LESSONS && LESSONS.length > 0) initialLessons.push(LESSONS[0]);
    if (LESSONS && LESSONS.length > 1) initialLessons.push(LESSONS[1]);
    if (LESSONS && LESSONS.length > 2) initialLessons.push(LESSONS[2]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 p-3 sm:p-6 lg:p-8" dir={isRtl ? "rtl" : "ltr"}>
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl shadow-xl shadow-purple-100 p-4 sm:p-6 border border-purple-50">
          ...
          <div className="space-y-2 sm:space-y-3">
            {lessons.map((lesson, index) => {
              ...
              return (
                <div key={index} className={`group bg-gradient-to-r ${gradientColors[gradientIndex]} rounded-xl sm:rounded-2xl p-3 sm:p-4 border hover:shadow-lg transition-all duration-300`}>
                  <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
                    ...
                    <div className="flex items-center justify-between sm:flex-col sm:items-end gap-2 sm:gap-2 pl-13 sm:pl-0">
                      <div className="flex gap-4 sm:flex-col sm:gap-2 sm:text-right">
                        <div>
                          <div className="text-[10px] sm:text-xs text-gray-500">{t("score_Dashboard")}</div>
                          <div className="font-bold text-gray-800 text-sm sm:text-base">{lesson.score || 0}</div>
                        </div>
                        <div>
                          <div className="text-[10px] sm:text-xs text-gray-500">{t("stars_Dashboard")}</div>
        ...
        <div className="space-y-4 sm:space-y-6">
          <div className="bg-white rounded-2xl sm:rounded-3xl shadow-xl shadow-purple-100 p-4 sm:p-6 border border-purple-50">
            ...
            <div className="bg-gradient-to-r from-purple-500 to-pink-500 rounded-xl sm:rounded-2xl p-3 sm:p-4 mb-3 sm:mb-4">
              <div className="text-white text-xs sm:text-sm mb-0.5 sm:mb-1">{t("currentSpeed_Dashboard")}</div>
              <div className="text-white text-2xl sm:text-3xl font-bold">36 <span className="text-base sm:text-lg">WPM</span></div>
            </div>
            <div className="space-y-2 sm:space-y-3">
              <div className="flex items-center justify-between py-2 sm:py-3 border-b border-gray-100">
                <span className="text-xs sm:text-sm text-gray-600">{t("lastWeek_Dashboard")}</span>
                <span className="font-bold text-gray-800 bg-gray-100 px-2 sm:px-3 py-0.5 sm:py-1 rounded-lg text-xs sm:text-sm">31 WPM</span>
              </div>
              <div className="flex items-center justify-between py-2 sm:py-3">
                <span className="text-xs sm:text-sm text-gray-600">{t("lastMonth_Dashboard")}</span>
                <span className="font-bold text-gray-800 bg-gray-100 px-2 sm:px-3 py-0.5 sm:py-1 rounded-lg text-xs sm:text-sm">27 WPM</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl sm:rounded-3xl shadow-xl shadow-purple-100 p-4 sm:p-6 border border-purple-50">
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              <div className="bg-gradient-to-br from-orange-50 to-yellow-50 rounded-xl sm:rounded-2xl p-3 sm:p-4 text-center border border-orange-100">
                <div className="text-2xl sm:text-3xl font-bold text-orange-500">74%</div>
                <div className="text-[10px] sm:text-xs text-gray-600 mt-0.5 sm:mt-1">{t("keyboardCoverage_Dashboard")}</div>
              </div>
              <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl sm:rounded-2xl p-3 sm:p-4 text-center border border-green-100">
                <div className="text-2xl sm:text-3xl font-bold text-green-500">94%</div>
                <div className="text-[10px] sm:text-xs text-gray-600 mt-0.5 sm:mt-1">{t("accuracy_Dashboard")}</div>
              </div>
            </div>
            <div className="flex items-center justify-between mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-gray-100">
              <span className="text-xs sm:text-sm text-gray-600">{t("totalActive_Dashboard")}</span>
              <span className="font-bold text-gray-800 bg-purple-100 px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg sm:rounded-xl text-purple-600 text-xs sm:text-sm">31h 40m</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto mt-4 sm:mt-6 grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        <div className="bg-white rounded-2xl sm:rounded-3xl shadow-xl shadow-purple-100 p-4 sm:p-6 border border-purple-50">
          <h3 className="text-base sm:text-lg font-bold text-gray-800 mb-3 sm:mb-4">
            {performanceTranslations[lang] || performanceTranslations.en}
          </h3>
          <ChartComponent />
        </div>
        {/* fdfa */}
